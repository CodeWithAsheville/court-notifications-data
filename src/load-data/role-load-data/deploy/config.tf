...
resource "aws_iam_role" "ct-load-data-role-$$INSTANCE$$" {
    name = "load-data-role-${var.instance}"
    assume_role_policy = file("./policy_role.json")
    tags = {
      Name          = "ct-load-data-role-${var.instance}"
...
resource "aws_iam_role_policy_attachment" "lambda_basic-$$INSTANCE$$" {
    role        = aws_iam_role.ct-load-data-role-$$INSTANCE$$.name
...
output "load_data_role_arn" {
  value = "${aws_iam_role.ct-load-data-role-$$INSTANCE$$.arn}"
}